...
def seed_data():
        ...
        for wh in ["VIC","NSW","SA"]:
            for i in range(50):
                rows.append({
                    ...
                    "Status": np.random.choice(["Open","Processing","Shipped","Invoiced"], p=[0.4,0.3,0.2,0.1]),
    ...
    if not os.path.exists(TOK_FILE):
        pd.DataFrame(columns=["token","role","company","expires_at","created_at"]).to_csv(TOK_FILE,index=False)
...
def generate_token(role="editor", hours=24):
    ...
    df.loc[len(df)] = {"token":token,"role":role,"company":CLIENT_COMPANY,
...
def dashboard_page():
    ...
    df = load_master()
    if mode=="client":
        # Clients get a copy (changes rollback on logout)
        df = df.copy()
    ...
    if role in ["editor","owner"]:
        ...
        if st.button("Apply Update"):
            ...
            if not idx: st.error("Order not found")
            else:
                i=idx[0]
